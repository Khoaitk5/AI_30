...
window.addEventListener("message", function (event) {
  ...
  if (event.data && event.data.type === "headerHeight") {
    // Lấy chiều cao từ header
    const headerHeightPx = event.data.height + "px";
    const headerHeightNum = event.data.height;

    // 1. SỬA SCROLLBAR: Đặt chiều cao cho chính iframe
    var headerIframe = document.querySelector(".header-iframe");
    if (headerIframe) {
      headerIframe.style.height = headerHeightPx;
    }

    // 2. Đặt padding cho nội dung (CHỈ KHI DROPDOWN KHÔNG MỞ)
    //    Chúng ta dùng 1 chiều cao "an toàn" (ví dụ 150px) để biết dropdown đang đóng
    var homeContent = document.querySelector(".font-svn-poppins");
    if (homeContent && headerHeightNum < 150) { 
      homeContent.style.paddingTop = headerHeightPx;
      homeContent.style.visibility = "visible";
      homeContent.style.opacity = 1;
    } else if (homeContent && !homeContent.style.paddingTop) {
      // Đảm bảo trang vẫn hiển thị nếu load lần đầu mà dropdown mở
      homeContent.style.paddingTop = "80px"; // Giá trị dự phòng
      homeContent.style.visibility = "visible";
      homeContent.style.opacity = 1;
    }
  // --- KẾT THÚC SỬA ---
      ...
      fetch("../login/login.html")
        ...
        .then((html) => {
          ...
          if (loginBox) {
            ...
            console.log(
              "Login modal loaded, calling initEyeToggle and initLoginForm"
            );
            initEyeToggle(loginBox);
            initLoginForm(loginBox);
          } else {
            console.log("Login box not found in html");
          }
          setTimeout(() => {
  ...
  } else if (event.data && event.data.type === "closeLoginModal") {
    var popup = document.getElementById("popup-login");
    if (popup) {
      popup.classList.add("hidden");
    }
    // ĐÃ XÓA CODE XỬ LÝ footerIframe TẠI ĐÂY
  } else if (event.data && event.data.type === "loginSuccess") {
    // Login success handled in login.js
    console.log('Home received loginSuccess');
...
// Listen for close login modal
window.addEventListener("closeLoginModal", function () {
  var popup = document.getElementById("popup-login");
  if (popup) {
    popup.classList.add("hidden");
  }
...
function initLoginForm(context = document) {
  const form = context.querySelector("form");
  if (form) {
    form.addEventListener("submit", async function (e) {
      e.preventDefault();
      console.log("Form submitted");
      const identifier = context.querySelector("#emailLogin").value; // Can be username, email, or phone
      const password = context.querySelector("#passwordLogin").value;
      ...
      try {
        ...
        if (response.ok) {
          // Login successful
          const userName =
            data.user.name || data.user.username || data.user.email || "User";
          console.log("Login successful, userName:", userName);
          // Send message to header iframe to update header
